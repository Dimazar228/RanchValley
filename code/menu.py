...
class Menu:
    ...
    def show_entry(self, text_surface, amount, top, selected):
        ...
        # background
        bg_rect = pygame.Rect(self.main_rect.left, top, self.width, text_surface.get_height() + (
                self.padding * 2))
        ...
        if selected:
            ...
            if self.index <= self.sell_border:  # sell
                ...
                pos_rect = self.buy_text.get_rect(midleft=(self.main_rect.left + 150,
                                                           bg_rect.centery))
    ...
    def update(self):
        ...
        for text_index, text_surface in enumerate(self.text_surfaces):
            top = self.main_rect.top + text_index * (text_surface.get_height() + (self.padding * 2) +
                                                     self.space)
